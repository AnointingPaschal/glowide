...
export async function POST(req: NextRequest) {
  try {
    const body = await req.json() as { action: string; walletId?: string; to?: string; amount?: string; tokenAddress?: string };
    const ciphertext = await getEntitySecretCiphertext();
    ...
    if (body.action === "transfer" && body.walletId && body.to && body.amount) {
      const { data, error } = await circleAPI<{ transfer: unknown }>(
