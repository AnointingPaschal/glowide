...
import { Bell, Command, Wifi, WifiOff, Code2, MessageSquare, Rocket, Search, Wallet, Settings, Home, Zap, Menu, X, Hammer } from "lucide-react";
import { WalletButton } from "@/components/wallet/WalletButton";
import { useState, useEffect } from "react";
import { cn } from "@/lib/utils";

// Admin route is excluded — access via /admin directly
const NAV_ITEMS = [
  ...
  return (
    <>
      <header className="h-12 md:h-14 border-b border-glow-border bg-glow-surface/90 backdrop-blur-md flex items-center gap-2 px-3 md:px-4 flex-shrink-0 z-30 relative">
        ...
        {/* Desktop Nav — full labels always visible on lg+ */}
        ...
        {/* Tablet Nav — icon + label on md-lg */}
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          <span className={cn(
            ...
            networkOk ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" : "bg-red-500/10 text-red-400 border-red-500/20"
          )}>
          ...
          <button onClick={() => setMobileOpen(!mobileOpen)} className="md:hidden p-1.5 rounded-lg text-glow-muted hover:text-glow-text hover:bg-glow-card transition-colors" aria-label="Menu">
              ...
              {NAV_ITEMS.map(({ href, icon: Icon, label }) => {
                ...
                return (
                  <Link key={href} href={href} className={cn(
                    ...
                    isActive ? "bg-glow-accent/20 text-glow-accent-light border border-glow-accent/20" : "text-glow-muted hover:text-glow-text hover:bg-glow-card"
                  )}>
                    <Icon className={cn("w-4 h-4 flex-shrink-0", isActive && "text-glow-accent-light")} />
                    {label}
                    {isActive && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-glow-accent animate-pulse" />}
                  </Link>
                );
              })}
            </nav>
