...
export function TopBar({ title: propTitle, description: propDesc }: { title?: string; description?: string } = {}) {
  ...
  const { lastCompileResult } = useEditorStore();

  // A contract is "ready to deploy" when the last compile succeeded and produced bytecode
      ...
      <header className="h-12 md:h-14 border-b border-[var(--glow-border)] bg-[var(--glow-surface)]/90 backdrop-blur-md flex items-center gap-2 px-3 md:px-4 flex-shrink-0 z-30 relative">
        ...
        <nav className="hidden lg:flex items-center gap-0.5 flex-1">
          {NAV_ITEMS.map(({ href, icon: Icon, label }) => {
            ...
            const isDeploy = href === "/deployments";
            return (
              <Link key={href} href={href} className={cn(
                "relative flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium transition-all whitespace-nowrap",
                ...
                {isDeploy && deployReady && (
                  <span className="absolute -top-1 -right-1 flex items-center justify-center w-4 h-4 rounded-full bg-emerald-500 text-white text-[9px] font-bold shadow-sm animate-pulse" title={`${lastCompileResult!.contractName} ready to deploy`}>
                    1
                  </span>
                )}
              </Link>
        ...
        <nav className="hidden md:flex lg:hidden items-center gap-0.5 flex-1">
          {NAV_ITEMS.map(({ href, icon: Icon, label }) => {
            ...
            const isDeploy = href === "/deployments";
            return (
              <Link key={href} href={href} title={isDeploy && deployReady ? `Deploy ${lastCompileResult!.contractName}` : label} className={cn(
                "relative flex items-center gap-1 px-2 py-1.5 rounded-lg text-xs font-medium transition-all",
                isActive ? "bg-[var(--glow-accent)]/20 text-[var(--glow-accent-light)]" : "text-[var(--glow-muted)] hover:text-[var(--glow-text)] hover:bg-[var(--glow-card)]"
              )}>
                <Icon className="w-3.5 h-3.5 flex-shrink-0" />
                {isDeploy && deployReady && (
                  <span className="absolute -top-1 -right-1 flex items-center justify-center w-4 h-4 rounded-full bg-emerald-500 text-white text-[9px] font-bold shadow-sm animate-pulse">
                    1
                  </span>
                )}
              </Link>
          ...
          <div className="relative w-64 max-w-[80vw] bg-[var(--glow-surface)] border-r border-[var(--glow-border)] flex flex-col shadow-card-shadow animate-slide-in-left">
            <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
              {NAV_ITEMS.map(({ href, icon: Icon, label }) => {
                ...
                return (
                  <Link key={href} href={href} className={cn(
                    "relative flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all text-sm font-medium",
                    ...
                    {isDeploy && deployReady && !isActive && (
                      <span className="ml-auto flex items-center gap-1 text-[10px] font-semibold text-emerald-400 bg-emerald-500/15 px-2 py-0.5 rounded-full">
                        <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse"/>
                        Ready
                      </span>
                    )}
                    {isDeploy && deployReady && isActive && (
                      <span className="ml-auto text-[10px] font-semibold text-emerald-400">Ready</span>
                    )}
                  </Link>
                );
              })}
            ...
            <div className="p-3 border-t border-[var(--glow-border)]">
              ...
              {deployReady && (
                <div className="mt-2 flex items-center gap-2 text-xs text-emerald-400 bg-emerald-500/10 rounded-lg px-2 py-1.5">
                  <Rocket className="w-3 h-3 flex-shrink-0"/>
                  <span className="font-medium truncate">{lastCompileResult!.contractName} ready to deploy</span>
                </div>
              )}
            </div>
          </div>
