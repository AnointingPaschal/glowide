...
import { X, ExternalLink, Download, Wifi, Smartphone, Check, AlertCircle, ChevronRight, Loader2 } from "lucide-react";
...
// ── Well-known wallets (shown even if not installed) ─────────────────
const KNOWN_WALLETS = [
  {
    id: "metamask",
    name: "MetaMask",
    ...
    detect: (p: WindowEthereum) => p.isMetaMask && !p.isBraveWallet && !p.isCoinbaseWallet,
  },
  {
    id: "coinbase",
    name: "Coinbase Wallet",
    icon: "https://www.coinbase.com/assets/press/coinbase-logos/coinbase-mark-primary.png",
    rdns: "com.coinbase.wallet",
    installUrl: "https://www.coinbase.com/wallet/downloads",
    deepLink: (url: string) => `https://go.cb-w.com/dapp?cb_url=${encodeURIComponent(url)}`,
    detect: (p: WindowEthereum) => !!(p.isCoinbaseWallet),
  },
  {
    id: "brave",
    name: "Brave Wallet",
    ...
    deepLink: null,
    detect: (p: WindowEthereum) => !!(p.isBraveWallet),
  },
  {
    id: "rainbow",
    name: "Rainbow",
    ...
    deepLink: (url: string) => `https://rnbwapp.com/dapp?url=${encodeURIComponent(url)}`,
    detect: (p: WindowEthereum) => !!(p.isRainbow),
  },
  {
    id: "trust",
    name: "Trust Wallet",
    ...
    detect: (p: WindowEthereum) => !!(p.isTrust),
  },
  {
    id: "phantom",
    name: "Phantom",
    ...
    deepLink: null,
    detect: (p: WindowEthereum) => !!(p.isPhantom),
  },
];
...
interface WalletModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConnect: (provider: WindowEthereum, walletName: string) => Promise<void>;
}
...
export function WalletModal({ isOpen, onClose, onConnect }: WalletModalProps) {
  ...
  useEffect(() => {
    ...
    // Fallback: legacy window.ethereum detection
    setTimeout(() => {
      if (wallets.length === 0 && window.ethereum) {
        const providerList = window.ethereum.providers ?? [window.ethereum];
        for (const p of providerList) {
          const known = KNOWN_WALLETS.find(kw => kw.detect(p));
          const id = known?.id ?? `injected-${Math.random().toString(36).slice(2)}`;
          if (seen.has(id)) continue;
          seen.add(id);
          wallets.push({
            id,
            name: known?.name ?? "Injected Wallet",
            icon: known?.icon ?? "",
            provider: p,
          });
        }
  ...
  // ── Handle connect ───────────────────────────────────────────────
  const handleConnect = useCallback(async (provider: WindowEthereum, walletName: string, walletId: string) => {
    setError(null);
    setConnecting(walletId);
    try {
      await onConnect(provider, walletName);
      onClose();
    } catch (err: unknown) {
      ...
      <div className="relative w-full max-w-sm bg-glow-card border border-glow-border rounded-2xl shadow-card-shadow animate-fade-in overflow-hidden">
        ...
        <div className="flex items-center justify-between p-4 border-b border-glow-border">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">Choose how to connect to GlowIDE</p>
          </div>
        ...
        <div className="p-4 space-y-3 max-h-[60vh] overflow-y-auto">
          ...
          {tab === "wallets" && (
            <>
                  ...
                  <div className="space-y-2">
                    {detected.map(w => (
                      <WalletRow
                        ...
                        badge="Installed"
                        badgeColor="emerald"
                        isConnecting={connecting === w.id}
                        onClick={() => handleConnect(w.provider, w.name, w.id)}
                      />
                    ))}
                  </div>
                </div>
              ...
              {/* Other known wallets (not detected) */}
              <div>
                <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-2">
                  {detected.length > 0 ? "More Wallets" : "No wallet detected — install one"}
                </p>
                <div className="space-y-2">
                  {KNOWN_WALLETS
                    .filter(kw => !detectedRdns.has(kw.rdns) && !detected.find(d => d.id === kw.id))
                    .map(kw => (
                      <WalletRow
                        key={kw.id}
                        name={kw.name}
              ...
              {detected.length === 0 && (
                <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                  <p className="text-xs text-amber-300">
                    No wallet extension detected. Install one above or use the <button className="underline" onClick={() => setTab("mobile")}>Mobile / QR</button> tab.
                  </p>
                </div>
              )}
            </>
          ...
          {tab === "mobile" && (
            <div className="space-y-3">
              <p className="text-xs text-gray-400">
                Open GlowIDE directly in your mobile wallet browser, or use a deep link to connect.
              </p>
              ...
              <div className="p-3 bg-glow-surface border border-glow-border rounded-xl">
                ...
                <p className="text-xs text-gray-500 mb-2">
                  Connect any WalletConnect-compatible wallet by scanning a QR code.
                  Requires <code className="text-glow-cyan">NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID</code> to be configured.
                </p>
                <a
                  href="https://walletconnect.com/explorer"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-xs text-glow-cyan underline"
                >
                  Browse 400+ supported wallets →
                </a>
              </div>
              ...
              <div className="p-3 bg-glow-bg border border-glow-border rounded-xl">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="text-xs text-glow-accent hover:text-glow-accent-light transition-colors flex-shrink-0"
                  >
                    Copy
                  </button>
                </div>
                <p className="text-[10px] text-gray-600 mt-1">Paste in your wallet's browser</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="px-4 py-3 border-t border-glow-border bg-glow-bg/50">
          <p className="text-[10px] text-gray-600 text-center">
            By connecting, you agree to GlowIDE&apos;s terms. Wallet keys never leave your device.
          </p>
        </div>
      </div>
    </div>
...
// ── Sub-components ───────────────────────────────────────────────────
function WalletRow({ name, icon, badge, badgeColor, isConnecting, onClick, href }: {
  name: string; icon: string; badge: string; badgeColor: "emerald" | "gray";
  isConnecting: boolean; onClick: () => void; href?: string;
}) {
  return (
    <button
      onClick={onClick}
      className="w-full flex items-center gap-3 p-3 bg-glow-surface border border-glow-border rounded-xl hover:border-glow-accent/40 hover:bg-glow-accent/5 transition-all group"
    >
      ...
      {isConnecting ? (
        ...
        <span className={cn(
          ...
          badgeColor === "emerald"
            ...
            : "text-gray-500 bg-glow-bg border-glow-border"
        )}>
          {badge}
        </span>
      )}
...
function WalletIcon({ icon, name }: { icon: string; name: string }) {
  if (!icon) {
    return (
      <div className="w-9 h-9 rounded-xl bg-glow-card border border-glow-border flex items-center justify-center text-sm font-bold text-glow-accent">
        {name[0]}
      </div>
    );
  }
  return (
    ...
    <img src={icon} alt={name} className="w-9 h-9 rounded-xl object-contain bg-white p-1" onError={e => { (e.target as HTMLImageElement).style.display = "none"; }} />
  );
}
